...
class Task:
	...
	def isFixed(self):
		...
		#runs the linux boolean command to see the result
			...
			return False
######################
users = [User(mainUser), User('jongun5'), User('aang'), User('barackobama'),User('vladimirputin'), User('xijinping'),User('sokka'),User('zuko') ] #If a user is deleted, you get a penalty
...
allTasks = [
	...
	Task('Returner','Returner Forensics 1', 5, '[ "$(grep 2.4.7 /home/'+ mainUser + '/Desktop/Forensics/Forensics2)" ]'), #
	Task('Returner','Dirbuster Removed', 3, ' ! [ "$(ls /home/cyber/Desktop | grep DirBuster)" ]'),
	#apache	
	...
	Task('Returner','Apache2 mod evasive installed', 3, ' [ "$(dpkg-query -l libapache2-mod-evasive)" ]'),
	Task('Returner','Apache2 Fail2ban installed', 3, ' [ "$(systemctl list-units --type=service | grep fail2ban)" ]'), 
	...
	Task('Returner','Apache2 bad website removed', 3, ' ! [ "$(ls /var/www/html | grep macdonald)" ]'),  #
	Task('Returner','Apache2 password file removed', 3, ' ! [ "$(ls /var/www/html/recipie-share | grep password)" ]'),  #
	Task('Returner','Apache2 port set', 3, ' [ "$(grep 443 /etc/apache2/ports.conf)" ]'), #
	Task('Returner','www-data has correct settings', 3, '! [ "$(grep www-data /etc/passwd | grep bash)" ]'), #
	Task('Returner','Apache2 keep alive settings', 3, '! [ "$(grep 100000 /etc/apache2/apache2.conf)" ]'),  #
	Task('Returner','Apache2 headers tls', 3, ' [ "$(apache2ctl -t -D DUMP_MODULES | grep header)" ]'),   
	Task('Returner','Apache2 CGI disabled', 3, '! [ "$(apache2ctl -t -D DUMP_MODULES | grep cgi)" ]'), #solved automatically
	Task('Returner','Apache2 added Request Size limit', 3, ' [ "$(grep LimitRequestBody /etc/apache2/apache2.conf)" ]'), #
	Task('Returner','Apache2 blocks bad IPs', 3, ' [ "$(grep "deny from 10.172.1.1" /var/www/html/recipie-share/.htaccess)" ]'),
	Task('Returner','Apache2 no directory browsing', 3, ' [ "$(grep "Options All -Indexes" /var/www/html/recipie-share/.htaccess)" ]'),
	Task('Returner','Apache2 permissions set', 3, ' ! [ "$(stat -c "%a %n" /etc/apache2/apache2.conf | grep 777)" ]'), #
	#mysql
	Task('Returner','Mysql correct port set', 2,'[ "$(grep 3306 /etc/mysql/my.cnf)" ]'),
	Task('Returner','Mysql local infile disabled', 2,'[ "$(grep local-infile=0 /etc/mysql/my.cnf)" ]'),
	Task('Returner','Mysql bind address set', 2,'[ "$(grep 127.0.0.1 /etc/mysql/my.cnf)" ]'),
	Task('Returner','MYSQL secure installation installed', 3, ' [ "$(dpkg-query -l mysql_secure_installation)" ]'),
	Task('Returner','my.cnf permissions set', 3, ' ! [ "$(stat -c "%a %n" /etc/mysql/my.cnf | grep 111)" ]'),
	Task('Returner','my.cnf packet size fixed', 3, ' ! [ "$(grep 16000 /etc/mysql/my.cnf)" ]'),
	Task('Returner','MYSQL history cleared', 3, ' ! [ "$(grep coolPassword ~/.mysql_history)" ]'),
	Task('Returner','mysql logs permissions set', 3, ' ! [ "$(stat -c "%a %n" /var/log/mysql | grep 777)" ]'),
	Task('Returner','Mysql not running as root', 3, ' ! [ "$(grep user=root /etc/mysql/my.cnf)" ]'),
	Task('Returner','Mysql local infile set', 3, ' [ "$(grep local-infile=0 /etc/mysql/my.cnf)" ]'),
	Task('Returner','Mysql TLS enabled set', 3, ' ! [ "$(grep require_secure_transport=OFF /etc/mysql/my.cnf)" ]'),
	Task('Returner','Mysql bad user removed', 3, ' ! [ "$(grep -a haxor /var/lib/mysql/mysql/user.MYD)" ]'),
	Task('Returner','Mysql started', 7, '  ! [ "$(service mysql status | grep stop)" ]')
	#php
	...
	#Task('Returner','PHP remote code execution blocked', 2,'[ "$(grep allow_url_fopen /etc/php/7.0/cli/php.ini | grep -i off)" ]'),
