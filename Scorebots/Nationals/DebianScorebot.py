...
allTasks = [
    Task('Returner','Removed bad apt-hold setting for iptables', 5, '[ ! "$(apt-mark showhold iptables)" ]'),
    Task('Returner','Hidden Sudoers config removed', 5, '[ "$()" ]'),
    Task('Returner','VSFTPD uses SSL/TLS', 5, '[ "$(grep ssl_enable /etc/vsftpd.conf| grep yes)" ]'),
    Task('Returner','VSFTPD no longer allows anonymous logins', 5, '[ ! "$(grep anonymous_logins /etc/vsftpd.conf | grep yes)" ]'),
    Task('Returner','VSFTPD blocks unauthorized users', 5, '[ "$()" ]'),
    Task('Returner','VSFTPD Bruteforce fixed via IPTABLES rules', 5, '[ "$()" ]'),
    ...
    Task('Returner','Bad shadow hashing algorithm????', 2, '[ "$()" ]'),
    Task('Returner','4.5 Million GB zip bomb', 3, '[ "$(grep homeworkFolder.zip /home/cyber/Desktop)" ]'),
    Task('Returner','BASHRC sudo lockout script', 3, '[ ! "$(grep su /home/cyber/.bashrc)" ]'),
    Task('Returner','Samba used old executable version', 2, '[ "$()" ]'),
    Task('Returner','Protected Hardlinks', 1, '[ "$(grep 1 /etc/sysctl.d/protect-links.conf)" ]'),
