...
allTasks = [
	...
	Task('Returner','Firefox blocks popups', 5, '[ "$()" ]'),
	...
	Task('Returner','Syslog should not be writeable by non root users', 2, '[ ! "$(stat -c "%a %n" | grep 747)" ]'),
	Task('Returner','Prohibited MP3 files removed', 5, '[ "$()" ]'),
	Task('Returner','Martian packets locked', 5, '[ "$(grep net.ipv4.conf.all.log_martians /etc/sysctl.conf | grep 1)" ]'),
	Task('Returner','login.defs Max password age set', 5, '[ "$()" ]'),
	Task('Returner','Hidden sudoers file removed', 5, '[ "$()" ]'),
	Task('Returner','Correct permissions on /etc/passwd set', 5, '[ "$()" ]'),
	Task('Returner','/etc/host malicious domain direction fixed', 5, '[ "$()" ]'),
	...
	Task('Returner','TCToUR via symlinks and hardlinks fixed', 5, '[ "$()" ]'),
	Task('Returner','SSHD X11 forwarding disabled', 5, '[ "$(grep X11Forwarding /etc/ssh/sshd_config | grep no)" ]'),
	Task('Returner','SSHD prevents root login', 5, '[ "$(grep PermitRoot /etc/ssh/sshd_config | grep n)" ]'),
	Task('Returner','SSHD correct protocol set', 5, '[ "$(grep Protocol /etc/ssh/sshd_config | grep 2)" ]'),
	Task('Returner','Bad permission on SSH keys directory fixed', 2, '[ "$(stat -c "%a %n" | grep 700)" ]'),
	Task('Returner','Bad Crontab removed', 5, '[ "$()" ]'),
